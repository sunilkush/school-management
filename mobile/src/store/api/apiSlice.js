...
export const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // ── Circulars — numbered notices with read-acknowledgement. Post-dates the archived app, so
    // these are new here rather than restored.
    // The reader's own inbox: only published circulars they are a recipient of, each carrying the
    // caller's own viewedAt/acknowledgedAt and a needsAcknowledgement flag.
    getMyCirculars: builder.query({
      query: () => ({ url: '/circulars/mine' }),
      providesTags: ['Circular'],
    }),
    // GET /circulars/:id is deliberately side-effecting — opening a circular is what records the
    // view. So the detail screen's own fetch IS the read receipt; there is no separate
    // "mark as read" call to forget to make.
    getCircular: builder.query({
      query: (id) => ({ url: `/circulars/${id}` }),
      providesTags: ['Circular'],
    }),
    // Acknowledging is a separate, deliberate act from viewing — the backend counts them
    // separately and only accepts this on a circular that actually asks for one.
    acknowledgeCircular: builder.mutation({
