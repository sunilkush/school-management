...
import { Modal, Form, Input, Select, InputNumber, Button, Space, Typography } from "antd";
...
const SubjectForm = ({ isOpen, onClose, editData = null }) => {
  ...
  return (
    <Modal
      ...
      title={
        <div>
          <Title level={4} style={{ marginBottom: 0 }}>{editData ? "Edit Subject" : "Create Subject"}</Title>
          <Text type="secondary">
            {roleName === "Super Admin" ? "Global subject available for all schools" : "Subject for your school & academic year"}
          </Text>
        </div>
      }
    >
      <Form form={form} layout="vertical" onFinish={onFinish} style={{ marginTop: 16 }}>
        <Form.Item label="Subject Name" name="name" rules={[{ required: true }]}>
          {roleName === "Super Admin" ? (
            <Select placeholder="Select subject">{SUBJECTS.map((s) => <Option key={s} value={s}>{s}</Option>)}</Select>
          ) : (
        ...
        <Space size="large" style={{ display: "flex" }}>
          <Form.Item label="Category" name="category" rules={[{ required: true }]} style={{ flex: 1 }}>
            <Select placeholder="Select category">{["Core","Elective","Language","Practical"].map(c => <Option key={c} value={c}>{c}</Option>)}</Select>
          </Form.Item>

          <Form.Item label="Type" name="type" rules={[{ required: true }]} style={{ flex: 1 }}>
            <Select placeholder="Select type">{["Theory","Practical","Both"].map(t => <Option key={t} value={t}>{t}</Option>)}</Select>
          </Form.Item>
        </Space>

        <Space size="large" style={{ display: "flex" }}>
          <Form.Item label="Max Marks" name="maxMarks" style={{ flex: 1 }}><InputNumber min={0} className="w-full" /></Form.Item>
          <Form.Item label="Pass Marks" name="passMarks" style={{ flex: 1 }}><InputNumber min={0} className="w-full" /></Form.Item>
        </Space>

        {roleName === "School Admin" && (
          <Form.Item label="Assign Teachers" name="assignedTeachers">
            <Select mode="multiple" placeholder="Select teachers" allowClear>
        ...
        <Form.Item>
          <Space style={{ justifyContent: "flex-end", width: "100%" }}>
            ...
            <Button type="primary" htmlType="submit" loading={loading}>{editData ? "Update Subject" : "Create Subject"}</Button>
          </Space>
        </Form.Item>
      </Form>
    </Modal>
