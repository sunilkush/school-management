...
export default function SchoolAdminTimetablePage() {
  ...
  const fromCopySections = useMemo(
    () =>
      masters.sections.filter(
        (s) => getId(s.schoolClassId) === fromCopyClassId,
      ),
  ...
  const save = async () => {
    try {
      ...
      if (editing)
        await dispatch(
          updateTimetableEntry({ id: editing._id, payload }),
        ).unwrap();
      else await dispatch(createTimetableEntry(payload)).unwrap();
      message.success("Timetable saved");
      setModalOpen(false);
      dispatch(fetchTimetable({ schoolId, academicYearId, ...filters }));
    } catch (e) {
  ...
  const remove = (entry) =>
    Modal.confirm({
      title: "Delete period?",
      onOk: async () => {
        await dispatch(deleteTimetableEntry(entry._id))
          .unwrap()
          .catch(message.error);
        dispatch(fetchTimetable({ schoolId, academicYearId, ...filters }));
      },
    ...
    dispatch(
      bulkSaveTimetable({
        ...
        entries: entries.map((e) => ({
          ...
          timeSlotId: getId(e.timeSlotId),
          subjectId: getId(e.subjectId),
          teacherId: getId(e.teacherId),
          roomId: getId(e.roomId),
          type: e.type,
          note: e.note,
        })),
      }),
      ...
      .unwrap()
      .then(() => message.success("Bulk saved"))
      .catch(message.error);
  const copyWeek = async () => {
    try {
      const values = await copyForm.validateFields();
      await dispatch(
        copyWeekTimetable({ schoolId, academicYearId, ...values }),
      ).unwrap();
    ...
    <Space direction="vertical" size="large" className="w-full">
      <Card className="shadow-sm">
        <Row justify="space-between" gutter={[16, 16]}>
          <Col>
            <Title level={3}>Timetable Planner</Title>
            <Text type="secondary">
              Build weekly class-section timetables, detect teacher and room
              conflicts, and publish reliable schedules.
            </Text>
          </Col>
          <Col>
            <Space wrap>
      ...
      <Card className="shadow-sm">
        {!academicYearId ? (
          ...
          <Empty description="Select class and section" />
        )}
      </Card>
      <Modal
        open={copyOpen}
        ...
        <Form form={copyForm} layout="vertical">
          ...
          >
            <Select
              ...
              onChange={() =>
                copyForm.setFieldValue("fromSectionId", undefined)
              }
            />
          </Form.Item>
          <Form.Item
          ...
          >
            <Select
              ...
              options={fromCopySections.map((s) => ({
                value: s._id,
                label: getName(s),
              }))}
            />
            ...
            <>
              ...
              >
                <Select
                  ...
                  options={masters.subjects.map((s) => ({
                    value: s._id,
                    label: getName(s),
                  }))}
                />
