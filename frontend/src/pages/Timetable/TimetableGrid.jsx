  ...
  return (
    <Card size="small" className="shadow-sm" styles={{ body: { padding: 10 } }}>
      <Space direction="vertical" size={4} className="w-full">
        <Space wrap>
          <Tag color={isNonTeaching ? "gold" : "blue"}>
            {isNonTeaching ? entry.type : getName(entry.subjectId, "Subject")}
          </Tag>
          {entry.roomId ? (
            <Tag color="purple">{getName(entry.roomId)}</Tag>
          ) : null}
        </Space>
        {!isNonTeaching && entry.teacherId ? (
          <Space size={6}>
            <Avatar size="small" src={entry.teacherId.avatar}>
              {getName(entry.teacherId, "T")[0]}
            </Avatar>
            <Text>{getName(entry.teacherId)}</Text>
          </Space>
        ) : (
        ...
        {showClass && !isNonTeaching ? (
          <Text type="secondary">
            {getName(entry.schoolClassId, "Class")} -{" "}
            {getName(entry.sectionId, "Section")}
          </Text>
        ) : null}
        {!readOnly ? (
          <Space size={4}>
...
export default function TimetableGrid({
  ...
  if (!timeSlots.length)
    return (
      <Empty description="Create time slots to start building the timetable" />
    );
  const columns = [
    {
      title: "Time",
    ...
    ...DAYS.map((day) => ({
      title: (
        <Space>
          <CalendarOutlined />
          {dayLabel(day)}
          {today === day ? <Tag color="green">Today</Tag> : null}
        </Space>
      ),
      dataIndex: day,
