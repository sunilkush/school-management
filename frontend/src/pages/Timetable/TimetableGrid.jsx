    ...
    return (
        <Card size="small" className="shadow-sm" styles={{ body: { padding: 10 } }}>
            <Space direction="vertical" size={4} className="w-full">
                ...
                {showClass && !isNonTeaching ? (
                    <Text type="secondary">{getName(entry.schoolClassId, "Class")} - {getName(entry.sectionId, "Section")}</Text>
                ) : null}
...
export default function TimetableGrid({ timeSlots = [], entries = [], readOnly = false, showClass = false, onAdd, onEdit, onDelete }) {
    ...
    const columns = [
        ...
        ...DAYS.map((day) => ({
            ...
            render: (_, row) => <TimetableCell entry={grid[row.slot._id]?.[day]} readOnly={readOnly} showClass={showClass} onAdd={() => onAdd?.({ dayOfWeek: day, timeSlotId: row.slot._id })} onEdit={() => onEdit?.(grid[row.slot._id]?.[day])} onDelete={() => onDelete?.(grid[row.slot._id]?.[day])} />,
        })),
    ];
