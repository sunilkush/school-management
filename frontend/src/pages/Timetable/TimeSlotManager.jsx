...
import { createTimeSlot, deleteTimeSlot, fetchTimeSlots, updateTimeSlot } from "../../features/timetableSlice";
    ...
    const save = async () => {
        try {
            ...
            if (editing) {
                ...
                message.success("Time slot saved"); setOpen(false);
            }
    ...
    return <Space direction="vertical" className="w-full" size="large">
        <Card className="shadow-sm">
            <Space className="w-full" direction="vertical">
                ...
                <Typography.Text type="secondary">Manage period, break, lunch, assembly, and activity slots for the active academic year.</Typography.Text>
                <Button
                    type="primary"
                    icon={<PlusOutlined />}
                ...
                columns={[
                    ...
                    { title: "Type", dataIndex: "type" },
                    {
                        ...
                        render: (_, r) => <Space>
                            ...
                            <Button danger
                                onClick={() => dispatch(deleteTimeSlot(r._id)).unwrap().catch(message.error)}>Delete</Button></Space>
                    }]} />
        </Card>
        <Modal
            open={open}
            ...
            <Form
                ...
                <Form.Item name="order"
                    label="Order" rules={[{ required: true }]}>
                    <InputNumber className="w-full" min={1} />
                </Form.Item>
                <Form.Item name="name" label="Name" rules={[{ required: true }]}><Input /></Form.Item>
                <Form.Item name="startTime" label="Start Time" rules={[{ required: true }]}><Input type="time" /></Form.Item>
                <Form.Item name="endTime" label="End Time" rules={[{ required: true }]}><Input type="time" /></Form.Item>
                <Form.Item name="type" label="Type" initialValue="period"><Select options={["period", "break", "lunch", "assembly", "activity"].map((t) => ({ value: t, label: t }))} /></Form.Item>
            </Form>
        </Modal>
    </Space>;
}
