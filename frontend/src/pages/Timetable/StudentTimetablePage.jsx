...
export default function StudentTimetablePage() {
  ...
  const { timeSlots, myStudentTimetable, loading } = useSelector(
    (s) => s.timetable,
  );
  ...
  useEffect(() => {
    if (schoolId && academicYearId) {
      ...
      dispatch(fetchMyStudentTimetable({ academicYearId }));
    }
  }, [dispatch, schoolId, academicYearId]);
  return (
    <Space direction="vertical" className="w-full" size="large">
      <Card>
        <Typography.Title level={3}>My Class Timetable</Typography.Title>
        <Typography.Text type="secondary">
          Readonly class-section weekly timetable.
        </Typography.Text>
      </Card>
      <Card>
        {!academicYearId ? (
