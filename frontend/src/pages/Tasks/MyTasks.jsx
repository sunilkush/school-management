...
const DueChip = ({ date, status }) => {
  ...
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:10,fontWeight:600,color,background:bg,padding:"2px 7px",borderRadius:6}}>
...
const AssignedByTag = ({ task, currentUserId, isDark }) => {
  ...
  if (!creator || (creator._id||creator) === currentUserId) return null;
  const name = creator.name || creator.email?.split("@")[0] || "Unknown";
  return (
    <div style={{display:"flex",alignItems:"center",gap:5,fontSize:10.5,color:isDark?"#64748B":"#94A3B8"}}>
      <Avatar size={14} icon={<UserOutlined/>}
        style={{background:`hsl(${Math.abs((name.charCodeAt(0)||65)*40)%360},50%,50%)`,fontSize:7}}>
        {name[0]?.toUpperCase()}
      </Avatar>
      <span>from {name}</span>
    </div>
...
const CardContent = ({ task, colColor, currentUserId, isDark, dragProps={}, isOverlay }) => {
  ...
  return (
    <div style={{
      ...
      boxShadow:   isOverlay ? "0 14px 44px rgba(0,0,0,0.24)" : "0 1px 3px rgba(0,0,0,0.04)",
    ...
    <div style={{flex:"1 1 240px",minWidth:240,maxWidth:340,display:"flex",flexDirection:"column"}}>
      <div style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          ...
          <span style={{fontWeight:700,fontSize:12.5,color:isDark?col.color:col.dark}}>{col.label}</span>
        </div>
        <span style={{
          fontWeight:800,fontSize:11,color:"#fff",
          background:col.color,width:20,height:20,borderRadius:"50%",
...
const MyTasks = () => {
  ...
  const handleDragEnd = useCallback(async ({ active, over }) => {
    ...
    try {
      await dispatch(updateTask({ id:taskId, status:targetStatus })).unwrap();
    } catch {
  ...
  const counts = useMemo(()=>({
    ...
    overdue:    myTasks.filter((t)=>t.dueDate&&dayjs(t.dueDate).isBefore(dayjs(),"day")&&t.status!=="done").length,
  }),[myTasks]);
  ...
  return (
    <>
        ...
        extra={
          <button onClick={()=>dispatch(fetchTasks())}
            style={{
              ...
              padding:"6px 14px",borderRadius:9,border:`1px solid ${border}`,
              background:cardBg,cursor:"pointer",fontSize:13,fontWeight:600,color:txtMut,
            }}>
            <ReloadOutlined style={{fontSize:12}}/>{!isMobile&&" Refresh"}
          </button>
        }
      />
      ...
      <div style={{padding:isMobile?"0 12px 24px":"0 20px 28px",background:pageBg,minHeight:"calc(100vh - 118px)"}}>
        ...
        <div className="mt-stats" style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
          {STATS.map((s)=>(
            <div key={s.label} style={{
              ...
              <div style={{width:38,height:38,borderRadius:10,flexShrink:0,background:s.bg,
                display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              border:`1px solid ${border}`,background:isDark?"#1A2438":"#F4F7FF",
              color:isDark?"#E2E8F0":"#0F172A",fontSize:13,outline:"none",
            }}
          />
          <Select value={filterPriority} onChange={setFilterPriority} style={{width:140}}
            options={[
        ...
        {/* Mobile column tabs */}
        {isMobile&&(
          <ColNav activeKey={mobileCol} onChange={setMobileCol} counts={counts} isDark={isDark} border={border}/>
        )}
        ...
        {loading&&myTasks.length===0 ? (
          ...
          <div style={{textAlign:"center",padding:"60px 20px"}}>
            <Empty description={<span style={{color:txtMut}}>No tasks assigned to you yet.</span>}/>
          </div>
        ) : (
          <DndContext sensors={sensors} collisionDetection={pointerWithin}
            ...
            <DragOverlay dropAnimation={{duration:160,easing:"ease"}}>
              {activeTask&&(
                <CardContent
                  task={activeTask}
                  colColor={COLUMNS.find((c)=>c.key===activeTask.status)?.color||"#2563EB"}
                  currentUserId={currentUserId}
                  isDark={isDark} isOverlay
                />
              )}
            </DragOverlay>
          </DndContext>
