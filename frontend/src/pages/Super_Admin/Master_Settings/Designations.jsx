...
/* ── Level badge colours ─────────────────────────────────────────── */
const LEVEL_STYLE = {
  Senior: { color: "var(--accent)", bg: "rgba(20,184,166,0.2)" },
  Mid: { color: "var(--primary)", bg: "rgba(219,234,254,0.2)" },
  Junior: { color: "var(--success)", bg: "rgba(220,252,231,0.2)" },
};

function LevelBadge({ level }) {
  const style = LEVEL_STYLE[level] || { color: "var(--text-secondary)", bg: "var(--surface-soft)" };
  return (
  ...
  return (
    <div style={pageWrapper}>
      ...
      <div style={{ ...statGrid(160), marginTop: 20 }}>
        ...
        <div style={statCard({ color: "var(--accent)" })}>
          <div>
            <div style={statLabel("var(--accent)")}>Senior Level</div>
            <div style={statValue("var(--accent)")}>{seniorCount}</div>
          </div>
          <IdcardOutlined style={{ fontSize: 26, color: "var(--accent)", opacity: 0.4 }} />
        </div>
      </div>
