...
import { Heart, UserCheck, UserX, School2, Search, RefreshCw, Filter } from "lucide-react";
import {
...
const Parents = () => {
  ...
  const allParents = useMemo(() => users.filter((u) => u?.role?.name === "Parent"), [users]);

  const parents = useMemo(() => {
    let data = [...allParents];
    if (selectedSchool !== "All") data = data.filter((p) => p.school?.name === selectedSchool);
    ...
    return data;
  }, [allParents, selectedSchool, search, statusFilter]);
  ...
  const handleBulkAction = async (activate) => {
    const targets = allParents.filter((r) => selectedRowKeys.includes(r._id) && r._id !== currentUser?._id);
    ...
    try {
      ...
      message.success(`${targets.length} parent(s) ${activate ? "activated" : "deactivated"}`);
      setSelectedRowKeys([]);
  ...
  const activeCount = allParents.filter((u) => u.isActive).length;
  const inactiveCount = totalCount - activeCount;
  const schoolCount = new Set(allParents.map((u) => u.school?.name).filter(Boolean)).size;
  ...
  return (
    <>
      <style>{tableHeadCss("parents-tbl")}</style>
      <div style={pageWrapper}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={iconWell(ACCENT, 44)}><Heart size={22} /></div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)" }}>Parent Management</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Manage parents across all schools</div>
          </div>
        </div>

        <div style={statGrid(160)}>
          {[
            { label: "Total Parents", value: totalCount, color: ACCENT, icon: <Heart size={18} /> },
        ...
        <div style={sectionPanel}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Showing <strong>{parents.length}</strong> of <strong>{totalCount}</strong></span>
          </div>

          {selectedRowKeys.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(244,63,94,0.06)", border: "1px solid rgba(244,63,94,0.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{selectedRowKeys.length} selected</span>
              <Popconfirm title={`Activate ${selectedRowKeys.length} parent(s)?`} onConfirm={() => handleBulkAction(true)} okText="Activate">
              ...
              <Popconfirm title={`Deactivate ${selectedRowKeys.length} parent(s)?`} onConfirm={() => handleBulkAction(false)} okText="Deactivate" okButtonProps={{ danger: true }}>
          ...
          <Spin spinning={!!loading}>
            <div className="parents-tbl">
              <Table
                ...
                locale={{ emptyText: <div style={emptyState}><Heart size={32} color="var(--text-muted)" style={{ margin: "0 auto 8px" }} /><div style={{ color: "var(--text-muted)", fontSize: 13 }}>No parents found</div></div> }}
              />
            </div>
          </Spin>
        </div>
