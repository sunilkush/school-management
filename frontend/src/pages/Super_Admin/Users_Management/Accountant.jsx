...
import { Calculator, UserCheck, UserX, School2, Search, RefreshCw, Filter } from "lucide-react";
import {
...
const Accountant = () => {
  ...
  const allAccountants = useMemo(() => users.filter((u) => u?.role?.name === "Accountant"), [users]);

  const accountants = useMemo(() => {
    let data = [...allAccountants];
    if (selectedSchool !== "All") data = data.filter((a) => a.school?.name === selectedSchool);
    ...
    return data;
  }, [allAccountants, selectedSchool, search, statusFilter]);
  ...
  const handleBulkAction = async (activate) => {
    const targets = allAccountants.filter((r) => selectedRowKeys.includes(r._id) && r._id !== currentUser?._id);
  ...
  const totalCount = allAccountants.length;
  const activeCount = allAccountants.filter((u) => u.isActive).length;
  const inactiveCount = totalCount - activeCount;
  const schoolCount = new Set(allAccountants.map((u) => u.school?.name).filter(Boolean)).size;
  ...
  return (
    <>
      <style>{tableHeadCss("accountant-tbl")}</style>
      <div style={pageWrapper}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={iconWell(ACCENT, 44)}><Calculator size={22} /></div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)" }}>Accountant Management</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Manage accountants across all schools</div>
          </div>
        </div>

        <div style={statGrid(160)}>
          {[
            { label: "Total Accountants", value: totalCount, color: ACCENT, icon: <Calculator size={18} /> },
            ...
            { label: "Schools", value: schoolCount, color: "#14B8A6", icon: <School2 size={18} /> },
          ].map((s) => (
        ...
        <div style={sectionPanel}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Showing <strong>{accountants.length}</strong> of <strong>{totalCount}</strong></span>
          </div>

          {selectedRowKeys.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{selectedRowKeys.length} selected</span>
              <Popconfirm title={`Activate ${selectedRowKeys.length} accountant(s)?`} onConfirm={() => handleBulkAction(true)} okText="Activate">
              ...
              <Popconfirm title={`Deactivate ${selectedRowKeys.length} accountant(s)?`} onConfirm={() => handleBulkAction(false)} okText="Deactivate" okButtonProps={{ danger: true }}>
          ...
          <Spin spinning={!!loading}>
            <div className="accountant-tbl">
              <Table
                ...
                locale={{ emptyText: <div style={emptyState}><Calculator size={32} color="var(--text-muted)" style={{ margin: "0 auto 8px" }} /><div style={{ color: "var(--text-muted)", fontSize: 13 }}>No accountants found</div></div> }}
              />
            </div>
          </Spin>
        </div>
