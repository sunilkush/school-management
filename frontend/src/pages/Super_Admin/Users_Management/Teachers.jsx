...
import { GraduationCap, Users, UserCheck, UserX, School2, Search, RefreshCw, Filter } from "lucide-react";
import {
...
const Teachers = () => {
  ...
  const teachers = useMemo(() => {
    ...
    if (selectedSchool !== "All") data = data.filter((t) => t.school?.name === selectedSchool);
    ...
    return data;
  }, [users, selectedSchool, search, statusFilter]);

  const allTeachers = useMemo(() => users.filter((u) => u?.role?.name === "Teacher"), [users]);
  ...
  const handleBulkAction = async (activate) => {
    const targets = allTeachers.filter((r) => selectedRowKeys.includes(r._id) && r._id !== currentUser?._id);
  ...
  const activeCount = allTeachers.filter((u) => u.isActive).length;
  const inactiveCount = totalCount - activeCount;
  const schoolCount = new Set(allTeachers.map((u) => u.school?.name).filter(Boolean)).size;
  ...
  return (
    <>
      <style>{tableHeadCss("teachers-tbl")}</style>
      <div style={pageWrapper}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={iconWell(ACCENT, 44)}><GraduationCap size={22} /></div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)" }}>Teacher Management</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Manage teachers across all schools</div>
          </div>
        </div>

        <div style={statGrid(160)}>
          {[
            { label: "Total Teachers", value: totalCount, color: ACCENT, icon: <GraduationCap size={18} /> },
            ...
            { label: "Schools", value: schoolCount, color: "#8B5CF6", icon: <School2 size={18} /> },
          ].map((s) => (
        ...
        <div style={sectionPanel}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Showing <strong>{teachers.length}</strong> of <strong>{totalCount}</strong></span>
          </div>

          {selectedRowKeys.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{selectedRowKeys.length} selected</span>
              <Popconfirm title={`Activate ${selectedRowKeys.length} teacher(s)?`} onConfirm={() => handleBulkAction(true)} okText="Activate">
              ...
              <Popconfirm title={`Deactivate ${selectedRowKeys.length} teacher(s)?`} onConfirm={() => handleBulkAction(false)} okText="Deactivate" okButtonProps={{ danger: true }}>
          ...
          <Spin spinning={!!loading}>
            <div className="teachers-tbl">
              <Table
                ...
                locale={{ emptyText: <div style={emptyState}><GraduationCap size={32} color="var(--text-muted)" style={{ margin: "0 auto 8px" }} /><div style={{ color: "var(--text-muted)", fontSize: 13 }}>No teachers found</div></div> }}
              />
            </div>
          </Spin>
        </div>
