...
import { BookOpen, UserCheck, UserX, School2, Search, RefreshCw, Filter } from "lucide-react";
import {
...
const Students = () => {
  ...
  const allStudents = useMemo(() => users.filter((u) => u?.role?.name === "Student"), [users]);

  const students = useMemo(() => {
    let data = [...allStudents];
    if (selectedSchool !== "All") data = data.filter((s) => s.school?.name === selectedSchool);
    ...
    return data;
  }, [allStudents, selectedSchool, search, statusFilter]);
  ...
  const handleBulkAction = async (activate) => {
    const targets = allStudents.filter((r) => selectedRowKeys.includes(r._id) && r._id !== currentUser?._id);
    ...
    try {
      ...
      message.success(`${targets.length} student(s) ${activate ? "activated" : "deactivated"}`);
      setSelectedRowKeys([]);
  ...
  const activeCount = allStudents.filter((u) => u.isActive).length;
  const inactiveCount = totalCount - activeCount;
  const schoolCount = new Set(allStudents.map((u) => u.school?.name).filter(Boolean)).size;
  ...
  return (
    <>
      <style>{tableHeadCss("students-tbl")}</style>
      <div style={pageWrapper}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={iconWell(ACCENT, 44)}><BookOpen size={22} /></div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)" }}>Student Management</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>Manage students across all schools</div>
          </div>
        </div>

        <div style={statGrid(160)}>
          {[
            { label: "Total Students", value: totalCount, color: ACCENT, icon: <BookOpen size={18} /> },
        ...
        <div style={sectionPanel}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Showing <strong>{students.length}</strong> of <strong>{totalCount}</strong></span>
          </div>

          {selectedRowKeys.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 8, padding: "8px 14px", marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{selectedRowKeys.length} selected</span>
              <Popconfirm title={`Activate ${selectedRowKeys.length} student(s)?`} onConfirm={() => handleBulkAction(true)} okText="Activate">
              ...
              <Popconfirm title={`Deactivate ${selectedRowKeys.length} student(s)?`} onConfirm={() => handleBulkAction(false)} okText="Deactivate" okButtonProps={{ danger: true }}>
          ...
          <Spin spinning={!!loading}>
            <div className="students-tbl">
              <Table
                ...
                locale={{ emptyText: <div style={emptyState}><BookOpen size={32} color="var(--text-muted)" style={{ margin: "0 auto 8px" }} /><div style={{ color: "var(--text-muted)", fontSize: 13 }}>No students found</div></div> }}
              />
            </div>
          </Spin>
        </div>
