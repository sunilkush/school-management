...
const Students = () => {
  ...
  if (error)
    return (
      <Text type="danger" className="block text-center mt-6">{error}</Text>
    );
    ...
    <div style={{ padding: 16 }}>
      <Card bordered={false} style={{ marginBottom: 16 }}>
        <Flex justify="space-between" align="center" wrap="wrap">
          <div>
            <Title level={4} style={{ marginBottom: 0 }}>Student Management</Title>
            <Text type="secondary">Super Admin can manage students across all schools</Text>
          </div>
          <Space>
      ...
      <Card bordered={false}>
        {selectedRows.length > 0 && (
          ...
          >
            ...
            <Popconfirm
              title={`Activate ${selectedRows.length} student(s)?`}
            ...
            <Popconfirm
              title={`Deactivate ${selectedRows.length} student(s)?`}
