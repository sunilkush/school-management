...
const LibraryMembers = () => {
  ...
  const historyColumns = [
    ...
    { title: "Due Date", dataIndex: "dueDate", width: 110, render: (d, r) => <span style={{ color: r.status === "Overdue" ? "var(--danger)" : "inherit" }}>{d}</span> },
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 90,
      render: (t) => (
        <span style={{ ...pill(MEMBER_TYPE_COLORS[t] || "var(--text-secondary)", `color-mix(in srgb, ${MEMBER_TYPE_COLORS[t] || "var(--text-secondary)"} 9%, transparent)`) }}>
          {t}
        </span>
      ),
    },
  ...
  return (
    <div style={pageWrapper}>
      ...
      <div style={statGrid(150)}>
        {[
          { label: "Total Members",   value: stats.total,   icon: <TeamOutlined />,      color: "var(--cyan)" },
          { label: "Active Borrowers", value: stats.active,  icon: <BookOutlined />,       color: "var(--accent)" },
