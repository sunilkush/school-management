  ...
  return (
    <div style={pageWrapper}>
      ...
      <div style={statGrid(160)}>
        <KpiCard icon={<HomeOutlined />}         label="Total Rooms"       value={kpis.totalRooms}               color="var(--accent)" onClick={() => navigate("/dashboard/hostelwarden/rooms")} />
        <KpiCard icon={<TeamOutlined />}          label="Total Capacity"    value={kpis.totalCapacity}            color="var(--primary)" />
        <KpiCard icon={<CheckCircleOutlined />}   label="Occupied Beds"     value={kpis.totalOccupied}            color="var(--success)" />
        <KpiCard icon={<ClockCircleOutlined />}   label="Vacant Beds"       value={kpis.vacantBeds}               color="var(--text-muted)" />
        <KpiCard icon={<UserOutlined />}          label="Students"          value={kpis.totalStudents}            color="var(--accent)" onClick={() => navigate("/dashboard/hostelwarden/allocations")} />
        <KpiCard icon={<ClockCircleOutlined />}   label="Pending Leaves"    value={kpis.pendingLeaves}            color="var(--warning)" sub="Awaiting approval" onClick={() => navigate("/dashboard/hostelwarden/leaves")} />
        <KpiCard icon={<AlertOutlined />}         label="On Leave Today"    value={kpis.leavesToday}              color="var(--warning)" />
        <KpiCard icon={<UserOutlined />}          label="Visitors Today"    value={kpis.visitorsToday}            color="var(--primary)" onClick={() => navigate("/dashboard/hostelwarden/visitors")} />
        <KpiCard icon={<WarningOutlined />}       label="Open Complaints"   value={kpis.openComplaints}           color="var(--danger)" onClick={() => navigate("/dashboard/hostelwarden/complaints")} sub={kpis.urgentComplaints > 0 ? `${kpis.urgentComplaints} urgent` : undefined} />
        <KpiCard icon={<CheckCircleOutlined />}   label="New Admissions"    value={kpis.newAdmissionsThisMonth}   color="var(--success)" sub="This month" />
      </div>
        ...
        <Progress
          ...
          strokeColor={
            kpis.occupancyRate > 90
              ? { from: "var(--danger-light)", to: "var(--danger)" }
              : kpis.occupancyRate > 70
                ? { from: "var(--warning-light)", to: "var(--warning)" }
      ...
      <div style={{ ...sectionPanel, padding: 20, marginBottom: 20 }}>
        ...
        <div style={statGrid(120)}>
          ...
          <ActionTile icon={<UserOutlined />}         label="Log Visitor"   path="/dashboard/hostelwarden/visitors"    color="var(--primary)" navigate={navigate} />
          <ActionTile icon={<WarningOutlined />}      label="Complaints"    path="/dashboard/hostelwarden/complaints"  color="var(--danger)" navigate={navigate} />
          <ActionTile icon={<CheckCircleOutlined />}  label="Attendance"    path="/dashboard/hostelwarden/hostel-attendance" color="var(--success)" navigate={navigate} />
          <ActionTile icon={<HomeOutlined />}         label="Rooms"         path="/dashboard/hostelwarden/rooms"       color="var(--accent)" navigate={navigate} />
          <ActionTile icon={<TeamOutlined />}         label="Allocations"   path="/dashboard/hostelwarden/allocations" color="var(--accent)" navigate={navigate} />
        </div>
      </div>
            ...
            {leaves.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={leaves} barSize={22}>
                  ...
                  <Tooltip />
                  <Bar dataKey="count" fill="var(--primary-light)" radius={[6, 6, 0, 0]} name="Leaves" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={compls.map((c) => ({ name: c._id, value: c.count }))} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({ name, value }) => `${name} (${value})`} labelLine={false}>
                    {compls.map((_, i) => <Cell key={i} fill={CATEGORICAL_COLORS[i % CATEGORICAL_COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                  <Legend />
                </PieChart>
